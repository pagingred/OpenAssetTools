#include "Game/IW4/Menu/RawLoaderMenuListIW4.h"
...
namespace
{
    TEST_CASE("RawLoaderMenuList(IW4): Returns no action when file is missing", "[iw4][menulist][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuListIW4(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("ui/test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuList(IW4): Can load menu list with one menu", "[iw4][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(IW4): Can load menu list with multiple menus", "[iw4][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(IW4): Can load empty menu list", "[iw4][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
