#include "Game/IW4/Menu/RawLoaderMenuDefIW4.h"
...
namespace
{
    TEST_CASE("RawLoaderMenuDef(IW4): Returns no action when file is missing", "[iw4][menu][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuDefIW4(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuDef(IW4): Can parse basic menu def", "[iw4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW4): Can parse menu def properties", "[iw4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW4): Returns failure when file has no menuDef", "[iw4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
