#include "Game/T5/Menu/RawLoaderMenuListT5.h"
...
namespace
{
    ...
    TEST_CASE("RawLoaderMenuList(T5): Returns no action when file is missing", "[t5][menulist][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuListT5(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("ui/test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuList(T5): Can load menu list with one menu", "[t5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(T5): Can load menu list with multiple menus", "[t5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(T5): Can load empty menu list", "[t5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
