#include "Game/T5/Menu/RawLoaderMenuDefT5.h"
...
namespace
{
    ...
    TEST_CASE("RawLoaderMenuDef(T5): Returns no action when file is missing", "[t5][menu][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuDefT5(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuDef(T5): Can parse basic menu def", "[t5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(T5): Can parse menu def properties", "[t5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(T5): Returns failure when file has no menuDef", "[t5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
