#include "Game/T4/Menu/RawLoaderMenuDefT4.h"
...
namespace
{
    ...
    TEST_CASE("RawLoaderMenuDef(T4): Returns no action when file is missing", "[t4][menu][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuDefT4(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuDef(T4): Can parse basic menu def", "[t4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(T4): Can parse menu def properties", "[t4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(T4): Returns failure when file has no menuDef", "[t4][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
