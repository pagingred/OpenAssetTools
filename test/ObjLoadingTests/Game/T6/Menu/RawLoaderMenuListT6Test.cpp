#include "Game/T6/Menu/RawLoaderMenuListT6.h"
...
namespace
{
    ...
    TEST_CASE("RawLoaderMenuList(T6): Returns no action when file is missing", "[t6][menulist][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuListT6(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("ui/test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuList(T6): Can load menu list with one menu", "[t6][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(T6): Can load menu list with multiple menus", "[t6][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(T6): Can load empty menu list", "[t6][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
