#include "Game/IW5/Menu/RawLoaderMenuDefIW5.h"
...
namespace
{
    TEST_CASE("RawLoaderMenuDef(IW5): Returns no action when file is missing", "[iw5][menu][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuDefIW5(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuDef(IW5): Can parse basic menu def", "[iw5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW5): Can parse menu def properties", "[iw5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW5): Returns failure when file has no menuDef", "[iw5][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
