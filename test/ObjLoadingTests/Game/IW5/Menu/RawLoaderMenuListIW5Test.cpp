#include "Game/IW5/Menu/RawLoaderMenuListIW5.h"
...
namespace
{
    TEST_CASE("RawLoaderMenuList(IW5): Returns no action when file is missing", "[iw5][menulist][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuListIW5(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("ui/test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuList(IW5): Can load menu list with one menu", "[iw5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(IW5): Can load menu list with multiple menus", "[iw5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuList(IW5): Can load empty menu list", "[iw5][menulist][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("ui/test.menu", R"(
{
