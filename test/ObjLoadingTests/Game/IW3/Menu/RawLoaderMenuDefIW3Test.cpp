#include "Game/IW3/Menu/RawLoaderMenuDefIW3.h"
...
namespace
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW3): Returns no action when file is missing", "[iw3][menu][assetloader]")
    {
        ...
        auto loader = menu::CreateRawLoaderMenuDefIW3(memory, searchPath);
        REQUIRE_FALSE(loader->CreateAsset("test.menu", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderMenuDef(IW3): Can parse basic menu def", "[iw3][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW3): Can parse menu def properties", "[iw3][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
    ...
    TEST_CASE("RawLoaderMenuDef(IW3): Returns failure when file has no menuDef", "[iw3][menu][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("test.menu", R"(
{
