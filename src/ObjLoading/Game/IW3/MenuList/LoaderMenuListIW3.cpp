...
#include "Game/IW3/XModel/XModelConstantsIW3.h"
#include "Game/IW3/XModel/JsonXModelIW3.h"
...
namespace
{
    ...
    {
    public:
        MenuListLoader(MemoryManager& memory, ISearchPath& searchPath, ZoneScriptStrings& scriptStrings)
        ...
        AssetCreationResult CreateAsset(const std::string& assetName, AssetCreationContext& context) override
        {
            ...
            auto* menuList = m_memory.Alloc<MenuList>();
            menuList->name = m_memory.Dup(assetName.c_str());

            m_materials.clear();
            m_surfaces.clear();

            AssetRegistration<AssetMenuList> registration(assetName, menuList);
            //if (!LoadFromFile(*file.m_stream, *menuList, context, registration))
            //{
    ...
    private:
        bool LoadFromFile(std::istream& jsonStream, MenuList& menuList, AssetCreationContext& context, AssetRegistration<AssetXModel>& registration)
        {
            try
            {
                ...
                {
                    con::error("Tried to load xmodel \"{}\" but did not find expected type material of version 1 or 2", menuList.name);
                    return false;
                }
                ...
                if (version == 1u)
                {
                    ...
                    con::warn("DEPRECATED: XModel {} is version 1 that made use of bad GLTF bone rotations.", menuList.name);
                }
                else
                {
                    m_gltf_bad_rotation_formulas = false;
                ...
                return false;
                //CreateXModelFromJson(jXModel, xmodel, context, registration);
            }
...
namespace menu_list
{
    std::unique_ptr<AssetCreator<AssetMenuList>> CreateLoaderIW3 (MemoryManager& memory, ISearchPath& searchPath, Zone& zone)
    {
        return std::make_unique<MenuListLoader>(memory, searchPath, zone.m_script_strings);
    }
} // namespace IW3
