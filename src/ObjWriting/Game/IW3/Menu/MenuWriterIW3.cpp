    ...
    private:
        void WriteStatementNaive(const statement_s* statement) const
        {
            ...
            for (auto i = 0uz; i < entryCount; i++)
            {
                ...
                else if (entry->data.op >= OP_DVARINT && entry->data.op <= OP_DVARSTRING)
                {
        ...
        void WriteStatementOperator(const statement_s* statement, size_t& currentPos, bool& spaceNext) const
        {
            ...
            else if (expEntry->data.op >= OP_DVARINT && expEntry->data.op <= OP_DVARSTRING)
            {
        ...
        void WriteItemData(const itemDef_s* item)
        {
            ...
            WriteStringProperty("onFocus", item->onFocus);
            WriteStringProperty("leaveFocus", item->leaveFocus);
            WriteStringProperty("mouseEnter", item->mouseEnter);
            WriteStringProperty("mouseExit", item->mouseExit);
            WriteStringProperty("mouseEnterText", item->mouseEnterText);
            WriteStringProperty("mouseExitText", item->mouseExitText);
            WriteStringProperty("action", item->action);
        ...
        void WriteMenuData(const menuDef_t* menu)
        {
            ...
            WriteStringProperty("onOpen", menu->onOpen);
            WriteStringProperty("onClose", menu->onClose);
            WriteStringProperty("onESC", menu->onESC);
